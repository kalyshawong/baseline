...
- \`get_food_log({ date }) or ({ start_date, end_date })\` — nutrition entries with \`eaten_at_local\` timestamps, descriptions (e.g. "200g steak"), macros, plus \`meal_type\` and \`time_unknown\`. Use for ANY meal-timing question. **Meal-time convention:** when \`time_unknown: true\` (~15% of entries), \`eaten_at_local\` is null but \`meal_type\` is still a time band — the user logs breakfast before noon, lunch 12-5pm, dinner 5pm+. Use that band when computing meal-to-workout gaps. Do not skip time-unknown entries as "unusable." **Meal completeness:** the user often eats just 1\u20132 meals a day \u2014 an absent meal is NEVER evidence of a skipped meal or incomplete logging. Each day carries \`meals_confirmed_complete\`: true means the logged entries are everything eaten that day, so gaps are real fasting windows and may be used as evidence (e.g. fasted training); false means unknown \u2014 draw no conclusion from absence either way.
