...
interface WorkoutNoteData {
  ...
  /** Explicit fasted answer — no food logged ≠ fasted, only this counts. Null = not answered. */
...
export function WorkoutNotesBlock({ source, workoutId }: Props) {
  ...
  // Tri-state: null = unanswered, true/false = answered. Feed the meal→GI
  // analyzer as factors; unanswered runs sit out rather than get assumed.
    ...
    (async () => {
      try {
        ...
        if (res.ok) {
          ...
          setFasted(data.preRunFasted ?? null);
        }
      } catch {
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      const body = note
        ? { narrative: trimmed, preRunBowel: bowel, preRunFasted: fasted }
        : { source, workoutId, narrative: trimmed, preRunBowel: bowel, preRunFasted: fasted };
      ...
      setFasted(saved.preRunFasted ?? null);
      setExpanded(false);
    } catch (e) {
  ...
  }, [draft, bowel, fasted, note, source, workoutId]);
    ...
    <div className="mt-3 space-y-2 border-t border-[var(--color-border)] pt-3">
      ...
      {isEditing ? (
        <>
          ...
          {(
            [
              ["Emptied stomach before this run?", bowel, setBowel],
              ["Fasted (no food in the 4h before)?", fasted, setFasted],
            ] as const
          ).map(([label, value, set]) => (
        ...
        <>
          ...
          {(note?.preRunBowel != null || note?.preRunFasted != null) && (
            <p className="text-xs text-[var(--color-text-muted)]">
              {[
                note?.preRunBowel != null &&
                  `Emptied stomach pre-run: ${note.preRunBowel ? "yes" : "no"}`,
                note?.preRunFasted != null &&
                  `Fasted: ${note.preRunFasted ? "yes" : "no"}`,
              ]
                .filter(Boolean)
                .join(" · ")}
            </p>
