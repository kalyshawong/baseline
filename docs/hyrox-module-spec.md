...
model HyroxSession {
  ...
  workoutSessionId String?    // optional link to WorkoutSession
  day             DateTime
  sessionType     String      // easy_run | tempo | intervals | long_run | strength | compromised | station_work | recovery
  duration        Int         // seconds
  distance        Float?      // meters (for run types)
  avgHr           Int?
  perceivedRpe    Int?        // 1-10
  notes           String?

  // For compromised sessions: embedded intervals
  intervalsJson   String?     // JSON of { runMeters, runSeconds, station, stationReps, stationSeconds }[]
...
export type Block = "accumulation" | "transmutation" | "realization" | "taper";

export function currentBlock(plan: HyroxPlan, today: Date): {
  block: Block;
  weekInBlock: number;
  daysToRace: number;
  volumeMultiplier: number;   // 1.0 = baseline, 0.5 = taper week 2
  intensityMultiplier: number;
} { ... }
```

Default schedule for a 53-day runway:
- Days 53-40 (2 weeks): **accumulation** — high volume, moderate intensity
- Days 39-26 (2 weeks): **transmutation** — high intensity, moderate volume (compromised running dominates)
- Days 25-15 (1.5 weeks): **realization** — race-pace rehearsals, station PRs
- Days 14-0 (2 weeks): **taper** — volume −50%, intensity preserved, sharpness sessions
...
New route: `/body/hyrox` (or nested section on `/body` if Hyrox plan is active)
...
1. Schema: add `HyroxPlan`, `HyroxSession`, `HyroxStationBenchmark` models; `prisma db push`
...
4. `src/lib/hyrox-blocks.ts`: block + week-in-block calculator
5. `src/lib/hyrox-session-recommender.ts`: daily session rule engine
6. `POST /api/hyrox/plan` — create plan from existing Hyrox goal (auto-links by goalId)
7. `GET /api/hyrox/today` — returns today's recommended session
8. Extend `buildCoachContext()`: add `hyrox_plan`, `hyrox_pace_gap` sections
9. Extend `goalSystemPromptSection()`: reference the new sections in the hyrox branch
10. Minimal UI: new card on `/body` page showing countdown + today's session + "start session" CTA

**Acceptance:** the coach can answer "what should I do today?" with a specific prescription, and "am I on pace?" with numbers pulled from logged sessions.
...
| System | Integration |
|---|---|
| **Goals** | HyroxPlan is keyed by `goalId` (unique). Setting a goal with `type=race, subtype=hyrox` auto-creates an empty plan. |
| **Workout logger** | HyroxSession can link to WorkoutSession via `workoutSessionId` so strength days show up in both views. |
...
1. **Plan start date** — should the plan always start "now" or should we allow back-dating? Suggest: allow back-dating via a "When did you start serious training?" field at creation.
2. **Solo vs Mixed Doubles distinction** — Mixed Doubles has pair mechanics (split stations, relay runs). V1 ignores this; treats it as solo pace target. Flag for V2.
3. **How strict should taper be?** — Research says 41-60% volume reduction. Default to 50% but allow user override.
4. **Where does the run logger live?** — Use existing running metric sync (once HAE issue is fixed), or let user manually log pace + distance as a HyroxSession. Suggest: both — auto-populate from Apple Watch when a run is detected within the plan's time window, allow manual edit.
